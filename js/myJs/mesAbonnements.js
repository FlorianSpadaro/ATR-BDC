                ...
                if(abo.secteur_id != null)
                    {
                        ...
                        $("#enteteSecteur" + abo.secteur_id + " .abonner:first").html('Se désabonner <span class="glyphicon glyphicon-remove"></span>').removeClass("abonner").addClass("desabonner").attr("id", abo.id);
                    }
                else if(abo.domaine_id != null)
                    {
                        $("#enteteDomaine" + abo.domaine_id).removeClass("panel-default").addClass("panel-success");
                        $("#enteteDomaine" + abo.domaine_id + " .abonner:first").html('Se désabonner <span class="glyphicon glyphicon-remove"></span>').removeClass("abonner").addClass("desabonner").attr("id", abo.id);
                    }
                else if(abo.sous_domaine_id != null)
                    {
                        $("#enteteSousDomaine" + abo.sous_domaine_id).removeClass("panel-default").addClass("panel-success");
                        $("#enteteSousDomaine" + abo.sous_domaine_id + " .abonner:first").html('Se désabonner <span class="glyphicon glyphicon-remove"></span>').removeClass("abonner").addClass("desabonner").attr("id", abo.id);
                    }
                else if(abo.projet_id != null)
                    {
                        $("#enteteProjet" + abo.projet_id).addClass("list-group-item-success");
                        $("#enteteProjet" + abo.projet_id + " .abonner:first").html('Se désabonner <span class="glyphicon glyphicon-remove"></span>').removeClass("abonner").addClass("desabonner").attr("id", abo.id);
                    }
                else if(abo.contrat_id != null){
                    $("#enteteContrat" + abo.contrat_id).addClass("list-group-item-success");
                    $("#enteteContrat" + abo.contrat_id + " .abonner:first").html('Se désabonner <span class="glyphicon glyphicon-remove"></span>').removeClass("abonner").addClass("desabonner").attr("id", abo.id);
                }
            });
        });
    }
